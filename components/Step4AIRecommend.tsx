...
interface Step4AIRecommendProps {
  report?: AccidentReport;
  criteria: AssessmentCriteria[];
  onSelectCriteria: (criteria: AssessmentCriteria) => void;
}

export default function Step4AIRecommend({
  report,
  criteria,
  onSelectCriteria,
}: Step4AIRecommendProps) {
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      ...
      alert("AI推奨の取得中にエラーが発生しました。もう一度お試しください。");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="h-full">
      <div className="flex flex-col h-full">
        <div className="mb-4">
          <h2 className="text-2xl font-bold mb-2">ステップ4: AI推奨基準の確認</h2>
              ...
              {aiRecommendations.map((rec, index) => (
                <div
                  ...
                  className={`p-4 border-2 rounded-lg transition-colors cursor-pointer ${
                    report?.selectedCriteria?.id === rec.criteria.id
                ...
                <div className="p-4 border border-yellow-200 bg-yellow-50 rounded-lg">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-yellow-300 rounded text-sm focus:ring-2 focus:ring-yellow-500 focus:border-transparent text-gray-900 bg-white"
                      onKeyDown={async (e) => {
                        if (e.key === "Enter") {
